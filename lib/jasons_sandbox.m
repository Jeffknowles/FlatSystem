...
WRITECOEFF=1;
path = 'C:\data\adata\';
fname = 'LFM_150_10_5_bk.bin';
%path='/DATA/AveragerDATA/';
%fname='za_cm_47.bin';
idx1 = 1:2500;
idx2 = 2335+(1:2500);
t0 = 100;
...
%files = findfiles('DATA/AveragerDATA/',patt);
%length(files)
...
for fnum = 1:length(files)
    fname = files(fnum).fname;
    path = files(fnum).path;
    
    
    % read data files
    res = read_arraybin([path fname],2)';
    %res = niLoadBin([fname '.bin'],path,2);
    ...
    Hdmag = filtfilt(ones(500,1)./500,1,Hdmag);        % use MA filter
...
%     ylabel('Amplitude')
    ...
    if WRITECOEFF
    FID=fopen('C:\data\filter\polaroid_filtercoeffs.bin','w','b');
    fwrite(FID,[a(:) b(:)],'float');
    fclose(FID);
    end
